...
def get_processed(type='small', dataset_root="/home/syzygianinfern0/sambashare/"):
    ...
    # Reading input
    input_img_list = [rawpy.imread(os.path.join(dataset_root, in_path)) for in_path in list(zip(*dataset))[0]]
    input_img_list = [pack_raw(input_img) * ratio for input_img in
                      input_img_list]  # (H,W,C)  C = 4
    input_thermal_list = [open(therm).read().strip().split('\n') for _, _, therm in zip(*dataset)]
    temps = []
